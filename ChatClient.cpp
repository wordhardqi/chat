...
ChatClient::ChatClient(
    ...
    const boost::asio::ip::basic_resolver<
        tcp, boost::asio::any_io_executor>::results_type &endpoints,
    size_t id)
    : client_id(id), io_context_(io_context), socket_(io_context),
...
void ChatClient::do_write() {
  ...
  if (front.header().msg_type == ChatMessage::msg_type_new) {
...
bool ChatClient::handle_msg() {
  ...
  case ChatMessage::msg_type_wait: {
    uint64_t src_seq;
    parse_uint64(read_msg_.body(), src_seq);
    msgs_tracker_.erase(src_seq);
    spdlog::info("no one is talking with you, plz wait");
    return true;
  }
  default: {
...
void ChatClient::print_msg(const uint64_t id, const uint64_t seq,
