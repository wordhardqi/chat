...
struct __attribute__((__packed__)) message_header {
  ...
  std::string to_string() const {
    return "len:" + std::to_string(msg_length) +
           " type:" + std::to_string(msg_type) +
           " src:" + std::to_string(msg_src) +
           " target: " + std::to_string(msg_target) +
...
public:
  ...
  static constexpr uint64_t msg_type_wait = 67UL;

  ChatMessage(uint64_t msg_type, uint64_t msg_src, uint64_t msg_seq)
      : data_(new std::vector<char>(header_length)) {
  ...
  ChatMessage(uint64_t msg_type, uint64_t msg_src, uint64_t msg_seq,
      ...
      : ChatMessage(msg_type, msg_src, msg_seq) {
  ...
  message_header &header() { return header_; }

  const char *body() const { return data() + header_length; }

  char *body() { return data() + header_length; }

  std::size_t body_length() const { return header_.msg_length - header_length; }
  ...
  bool decode_header() {
    ...
    const char *buf = data();
    std::memcpy(reinterpret_cast<void *>(&header_),
                reinterpret_cast<const void *>(buf), header_length);
    ...
    case msg_type_ack: {
      if (header_.msg_length != header_length + size_of_uint64) {
        spdlog::warn("cannot parse msg, msg_type {}", header_.msg_type);
        return false;
      }
      return true;
    }
  ...
  void encode_header() {
    ...
    memcpy(reinterpret_cast<void *>(buf),
           reinterpret_cast<const void *>(&header_), header_length);
  }
...
public:
    ...
    if (send_time_.size() > max_tracking_message) {
      send_time_.erase(send_time_.begin(),
                       std::next(send_time_.begin(),
                                 static_cast<int64_t>(send_time_.size() -
                                                      max_tracking_message)));
    }
  }
  ...
  void erase(uint64_t seq) { send_time_.erase(seq); }

private:
  std::map<uint64_t, int64_t>
      send_time_; // message seq to send_time, used for calulating rtt.
  std::map<uint64_t, int64_t>
      rtt_records_; // message seq to rtt time in nanoseconds.
};
